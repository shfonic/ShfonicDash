...
- **[src/core/config_store.py](src/core/config_store.py)** — Persists user settings (theme, units, flip, show_cursor, share_logs, share_window_days, show_session_summary, enabled_games, show_record_button, show_sync_button, web_app_mode, etc.) to a local JSON file. `enabled_games` (settings GAMES tab) filters which games the menu shows — missing ids default to enabled; the menu grid adapts to the enabled count. `share_window_days()` reads the share window with migration from the retired `log_retention_days` key. `web_app_mode()` (COMPANION tab, `off`/`menu`/`always`, default `menu`) controls when the browser web companion is reachable — see the web-companion bullet below. `show_sync_button` (COMPANION tab, default off) gates the game-menu SYNC pill — the Pythonista-companion sharing toggle, hidden by default so the general public never sees it (mirrors `show_record_button`). **Settings tabs:** DISPLAY / UNITS always; GAMES / COMPANION / DATA on the game menu only. COMPANION holds the web-companion mode + pairing URL/QR and the Pythonista SYNC section (Show SYNC button, Share logs over Wi-Fi, share window); DATA is session-data management (REBUILD SESSION INDEX + Show RECORD button).
